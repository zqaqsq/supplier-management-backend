...
$initScript = Get-Content "init.sql" | mysql -h $MYSQL_HOST -P $MYSQL_PORT -u $MYSQL_USER -p$MYSQL_PASSWORD_PLAIN supplier_management
if ($LASTEXITCODE -ne 0) {
...
$connectionUrl = "jdbc:mysql://$MYSQL_HOST`:$MYSQL_PORT/supplier_management?useUnicode=true&characterEncoding=utf8&useSSL=false&serverTimezone=GMT%2B8&allowPublicKeyRetrieval=true"
Write-Host "URL: $connectionUrl" -ForegroundColor Cyan
