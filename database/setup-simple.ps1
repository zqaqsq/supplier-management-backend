...
$MYSQL_PASSWORD = Read-Host "请输入MySQL密码"
...
$testConnection = & $MYSQL_PATH -h $MYSQL_HOST -P $MYSQL_PORT -u $MYSQL_USER -p$MYSQL_PASSWORD -e "SELECT 1;" 2>$null
...
$createDb = & $MYSQL_PATH -h $MYSQL_HOST -P $MYSQL_PORT -u $MYSQL_USER -p$MYSQL_PASSWORD -e "CREATE DATABASE IF NOT EXISTS supplier_management CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>$null
if ($LASTEXITCODE -ne 0) {
...
if (Test-Path $scriptPath) {
    $initScript = & $MYSQL_PATH -h $MYSQL_HOST -P $MYSQL_PORT -u $MYSQL_USER -p$MYSQL_PASSWORD supplier_management < $scriptPath
    if ($LASTEXITCODE -ne 0) {
...
Write-Host "密码: $MYSQL_PASSWORD" -ForegroundColor Cyan
Write-Host ""
