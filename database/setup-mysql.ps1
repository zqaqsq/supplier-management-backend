# 供应商管理系统 - MySQL数据库设置脚本 (Windows专用版本)
...
# 设置MySQL路径
$MYSQL_PATH = "C:\Program Files\MySQL\MySQL Server 9.2\bin\mysql.exe"
$MYSQLDUMP_PATH = "C:\Program Files\MySQL\MySQL Server 9.2\bin\mysqldump.exe"

# 检查MySQL是否安装
Write-Host "正在检查MySQL安装..." -ForegroundColor Yellow
if (Test-Path $MYSQL_PATH) {
    $mysqlVersion = & $MYSQL_PATH --version
    Write-Host "MySQL已安装，版本信息：" -ForegroundColor Green
    Write-Host $mysqlVersion -ForegroundColor Cyan
} else {
    Write-Host "错误：未找到MySQL安装，请检查路径：$MYSQL_PATH" -ForegroundColor Red
    Read-Host "按任意键退出"
    exit 1
}
...
$testConnection = & $MYSQL_PATH -h $MYSQL_HOST -P $MYSQL_PORT -u $MYSQL_USER -p$MYSQL_PASSWORD_PLAIN -e "SELECT 1;" 2>$null
...
$createDb = & $MYSQL_PATH -h $MYSQL_HOST -P $MYSQL_PORT -u $MYSQL_USER -p$MYSQL_PASSWORD_PLAIN -e "CREATE DATABASE IF NOT EXISTS supplier_management CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>$null
if ($LASTEXITCODE -ne 0) {
...
$scriptPath = Join-Path $PSScriptRoot "init.sql"
if (Test-Path $scriptPath) {
    $initScript = & $MYSQL_PATH -h $MYSQL_HOST -P $MYSQL_PORT -u $MYSQL_USER -p$MYSQL_PASSWORD_PLAIN supplier_management < $scriptPath
    if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "错误：未找到初始化脚本：$scriptPath" -ForegroundColor Red
    Read-Host "按任意键退出"
    exit 1
}
